...
Five individual assignments on **Project Pulse**, the instructor's own application and the codebase demonstrated in lecture. Each follows the shape of real work on an unfamiliar codebase: open an issue, make a change, submit a pull request. You work in your own fork of a class copy of the repository, one per assignment. Each is framed around the AI workflow and **graded on judgment, not on whether it ran**. These are individual work; see [Working with AI](ai.md) for what that means when you have an agent.
